...
async def get_schedules(
    ...
    # 只返回未来的排课
    query = query.where(CourseSchedule.start_time >= datetime.now(timezone.utc).replace(tzinfo=None).replace(tzinfo=None))
...
async def cancel_booking(
    ...
    booking.cancelled_at = datetime.now(timezone.utc).replace(tzinfo=None).replace(tzinfo=None)
...
async def check_in(
    ...
    booking.check_in_time = datetime.now(timezone.utc).replace(tzinfo=None).replace(tzinfo=None)
    ...
    return MiniBookingItem(
        ...
        course_name=course_name,
        coach_name=coach_name,
        start_time=schedule.start_time if schedule else datetime.now(timezone.utc).replace(tzinfo=None).replace(tzinfo=None),
        end_time=schedule.end_time if schedule else datetime.now(timezone.utc).replace(tzinfo=None).replace(tzinfo=None),
        status=booking.status.value,
...
async def get_today_bookings(
    ...
    today_start = datetime.now(timezone.utc).replace(tzinfo=None).replace(tzinfo=None).replace(hour=0, minute=0, second=0, microsecond=0)
